...
### 2️⃣ 기술 면접 질문 은행 (`/interview`)
- **실전 대비 특화**: 초급/중급/고급 난이도 및 빈출도별 선별된 핵심 질문 제공
- **반응형 아코디언**: 두괄식 모범 답안과 예상 꼬리 질문을 스스로 테스트할 수 있는 학습 인터페이스
- **A4 최적화 인쇄 (`@media print`)**: 군더더기 없는 흑백 레이아웃으로 변환하여 오프라인 암기용 종이 문서 출력(PDF 저장) 완벽 지원 
